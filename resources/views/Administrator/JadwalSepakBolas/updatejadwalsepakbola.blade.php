...
<div class="pagetitle">
    <h1>Ubah Data Jadwal Sepak Bola</h1>
    <nav>
      <ol class="breadcrumb">
        <li class="breadcrumb-item">Jadwal</li>
        <li class="breadcrumb-item"><a href="{{ route('JadwalSepakBola') }}"></a>Jadwal Sepak Bola</li>
        <li class="breadcrumb-item active">Ubah Data Jadwal Sepak Bola</li>
      </ol>
    </nav>
</div><!-- End Page Title -->
...
<div class="col-lg-12">
    ...
    <div class="card">
      <div class="card-body"><br>
        <form class="row g-3" action="{{ route('updateJadwalSepakBola', $data->id) }}" method="POST">
        @csrf
        <div class="col-12">
              ...
              <select id="id_stadion" class="form-control" name="id_stadion" required>
                <option value="">-- Select --</option>
                  @foreach ($data_stadion as $row)
                    <option value="{{ $row->id }}" {{ $row->id == $data->id_stadion ? 'selected' : '' }}>{{ $row->nama_stadion }}
                  @endforeach
              </select> 
          </div> 
