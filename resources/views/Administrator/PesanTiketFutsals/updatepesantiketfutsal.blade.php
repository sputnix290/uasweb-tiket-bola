...
<div class="pagetitle">
    <h1>Ubah Data Pesan Tiket Futsal</h1>
    <nav>
      <ol class="breadcrumb">
        <li class="breadcrumb-item">Pesan Tiket</li>
        <li class="breadcrumb-item"><a href="{{ route('PesanTiketFutsal') }}"></a>Pesan Tiket Futsal</li>
        <li class="breadcrumb-item active">Ubah Data Pesan Tiket Futsal</li>
      </ol>
    </nav>
</div><!-- End Page Title -->
...
<div class="col-lg-12">
    ...
    <div class="card">
      <div class="card-body"><br>
        <form class="row g-3" action="{{ route('updatePesanTiketFutsal', $data->id) }}" method="POST">
        @csrf
        <div class="col-12">
          ...
          <select id="id_tiket_futsal" class="form-control" name="id_tiket_futsal" required>
              ...
              @foreach ($data_tiketfutsal as $row) 
                <option value="{{ $row->id }}" {{ $row->id == $data->id_tiket_futsal ? 'selected' : '' }}>{{ $row->id }}
              @endforeach
          </select>
        </div>
        <div class="col-12">
